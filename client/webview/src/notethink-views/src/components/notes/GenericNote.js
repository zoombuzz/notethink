...
export default React.memo(function GenericNote(props) {
    const note = props;
    ...
    if (deepest_selectable_note.level === note.display_options?.deepest?.selectable_level) {
        ...
        cropped_selected_seqs = (note.display_options?.selected_notes || [])
            .filter((selected_note) => selected_note.level <= deepest_selectable_note.level)
            .map((selected_note) => selected_note.seq);
    }
    ...
    switch (props.type) {
        case 'list':
        case 'listItem':
            return _jsx(GenericNoteWrapper, { type: props.type, ...enriched_props });
        case 'code':
            switch (props.lang) {
                case 'mermaid':
                    return _jsx(MermaidNote, { ...enriched_props });
                default:
                    return _jsx(CodeNote, { ...enriched_props });
            }
    }
    return _jsx(MarkdownNote, { ...enriched_props });
});
