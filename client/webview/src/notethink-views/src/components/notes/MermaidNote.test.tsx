...
jest.mock('./MermaidDiagram', () => ({
    MermaidDiagram: (props: { children: string; className?: string; onClick?: () => void; onError?: () => void }) => (
        <div data-testid="mermaid-diagram" className={props.className} onClick={props.onClick}>
            {props.children}
        </div>
    ),
}));
...
describe('MermaidNote', () => {
    ...
    it('applies focused class when note is focused', () => {
        ...
        const { container } = render(<MermaidNote {...note} />);
        const note_element = container.firstChild as HTMLElement;
