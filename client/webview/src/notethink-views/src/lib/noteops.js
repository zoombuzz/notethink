 ...
 */
export function withinNoteHeadlineOrBody(pos, note) {
    if (pos === undefined) {
        return false;
    }
 ...
 */
export function withinNoteHeadlineOrBodyUpTo(pos, note, end_offset) {
    if (pos === undefined) {
        return false;
    }
 ...
 */
export function findDeepestNote(notes, caret_position, max_level) {
 ...
 */
export function findSelectedNotes(notes, selection) {
    return notes
 ...
 */
export function selectionSpans(selection, from, to) {
    if (!selection) {
        return false;
    }
 ...
 */
export function aggregateNoteLinetags(notes) {
 ...
 */
export function resolveCaretPosition(ncp, note) {
    return ncp.from;
}
/**
 * Calculate text changes for a checkbox action.
 */
export function calculateTextChangesForCheckbox(note, action_is_check, match_text, match_context) {
    const changes = [];
    ...
    if (match_position === -1 || checkbox_start_position === -1) {
        return [];
    }
    changes.push({
 ...
 */
export function standardNoteOrder(a, b) {
    return a.position.start.offset - b.position.start.offset;
}
/**
 * Kanban-specific note ordering by ordering weight then seq.
 */
export function kanbanNoteOrder(a, b) {
