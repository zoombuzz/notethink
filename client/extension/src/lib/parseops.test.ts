...
interface TomlNode { type: 'toml'; value: string }
...
describe('parseops', () => {
	describe('parse()', () => {
		...
		it('parses a heading node', () => {
			...
			expect((heading as Heading).depth).toBe(1);
		});

		it('parses nested headings', () => {
			const result = parse('# H1\n## H2\n### H3');
			const headings = result.children.filter((c: RootContent) => c.type === 'heading') as Heading[];
		...
		it('parses a paragraph with inline elements', () => {
			...
			const strong = para.children.find((c: PhrasingContent) => c.type === 'strong');
			expect(strong).toBeDefined();
			const emphasis = para.children.find((c: PhrasingContent) => c.type === 'emphasis');
		...
		it('parses GFM tables', () => {
			...
			const table = result.children.find((c: RootContent) => c.type === 'table');
		...
		it('parses GFM strikethrough', () => {
			...
			const result = parse(markdown);
			const para = result.children[0] as Paragraph;
			const del = para.children.find((c: PhrasingContent) => c.type === 'delete');
		...
		it('parses GFM autolinks', () => {
			...
			const result = parse(markdown);
			const para = result.children[0] as Paragraph;
			const link = para.children.find((c: PhrasingContent) => c.type === 'link') as Link | undefined;
		...
		it('parses YAML frontmatter', () => {
			const markdown = '---\ntitle: Test\ndate: 2024-01-01\n---\n\n# Content';
			const result = parse(markdown);
			const fm = result.children.find((c: RootContent) => c.type === 'yaml') as Yaml | undefined;
		...
		it('parses TOML frontmatter', () => {
			const markdown = '+++\ntitle = "Test"\n+++\n\n# Content';
			const result = parse(markdown);
			// TOML frontmatter isn't part of the standard mdast type tree, so cast through unknown
			const fm = result.children.find((c: RootContent) => (c as unknown as TomlNode).type === 'toml') as unknown as TomlNode | undefined;
			expect(fm).toBeDefined();
			expect(fm?.value).toContain('title = "Test"');
		});
		...
		it('parses fenced code blocks', () => {
			const markdown = '```js\nconsole.log("hi");\n```';
			const result = parse(markdown);
			const code = result.children.find((c: RootContent) => c.type === 'code') as Code | undefined;
		...
		it('parses images', () => {
			...
			const result = parse(markdown);
			const para = result.children[0] as Paragraph;
			const img = para.children.find((c: PhrasingContent) => c.type === 'image') as Image | undefined;
	...
	describe('unicode characters', () => {
		...
		it('parses CJK characters in content', () => {
			...
			const heading = result.children[0] as Heading;
			expect(heading.type).toBe('heading');
			const paras = result.children.filter((c: RootContent) => c.type === 'paragraph');
		...
		it('parses accented and diacritical characters', () => {
			...
			const heading = result.children[0] as Heading;
			expect(heading.type).toBe('heading');
			expect((heading.children[0] as Text).value).toBe('Ñoño résumé naïve über café');
		});
		...
		it('parses RTL text (Arabic/Hebrew)', () => {
			...
			const heading = result.children[0] as Heading;
			expect(heading.type).toBe('heading');
			const para = result.children[1] as Paragraph;
			expect(para.type).toBe('paragraph');
		});
		...
		it('parses GFM tables with unicode cell content', () => {
			...
			const result = parse(markdown);
			const table = result.children.find((c: RootContent) => c.type === 'table') as Table | undefined;
			expect(table).toBeDefined();
		});
	...
	describe('GFM footnotes', () => {
		it('parses footnote references and definitions', () => {
			...
			const result = parse(markdown);
			const footnote_ref = result.children.find((c: RootContent) => c.type === 'paragraph');
			expect(footnote_ref).toBeDefined();
			const footnote_def = result.children.find((c: RootContent) => c.type === 'footnoteDefinition') as FootnoteDefinition | undefined;
			expect(footnote_def).toBeDefined();
			expect(footnote_def?.identifier).toBe('1');
		});

		it('parses multiple footnotes', () => {
			const markdown = 'First[^a] and second[^b].\n\n[^a]: Note A.\n\n[^b]: Note B.';
			const result = parse(markdown);
			const defs = result.children.filter((c: RootContent) => c.type === 'footnoteDefinition');
		...
		it('parses footnote with multi-line content', () => {
			...
			const result = parse(markdown);
			const def = result.children.find((c: RootContent) => c.type === 'footnoteDefinition') as FootnoteDefinition | undefined;
	...
	describe('large file (1000+ lines)', () => {
		it('parses a 1500-line markdown file correctly', () => {
			...
			const result = parse(markdown);
			expect(result.type).toBe('root');
			const headings = result.children.filter((c: RootContent) => c.type === 'heading');
	...
	describe('mixed content document', () => {
		it('parses frontmatter + GFM features + unicode in one document', () => {
			...
			const types = result.children.map((c: RootContent) => c.type);
