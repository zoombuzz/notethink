...
function makePosition(offset, text) {
    let line = 1;
 ...
 */
function computeHeadingEndBody(headingIndex, siblings, parentEndOffset) {
 ...
 */
function extractHeadlineRaw(node, text) {
 ...
 */
function extractBodyRaw(nodeEndOffset, endBodyOffset, text) {
 ...
 */
function isListItemChildNote(node, parentType) {
 ...
 */
function findChildNotes(parentType, children, text, seqCounter, parentEndOffset, allNotes, childrenBodyAccumulator) {
    for (let i = 0; i < children.length; i++) {
        ...
        if (child.type === 'heading' && end_body_offset !== undefined) {
            ...
            for (let j = i + 1; j < children.length; j++) {
                ...
                if (sibling.position.start.offset >= end_body_offset) {
                    break;
                }
            ...
            if (sectionChildren.length > 0) {
                findChildNotes(child.type, sectionChildren, text, seqCounter, end_body_offset, allNotes, noteChildrenBody);
            }
        }
        else if ((child.type === 'list' || child.type === 'listItem') && child.children?.length) {
            findChildNotes(child.type, child.children, text, seqCounter, child.position.end.offset, allNotes, noteChildrenBody);
        }
    }
}
/**
 ...
 */
function subsumedBy(noteA, noteB) {
 ...
 */
function nestChildNotes(allNotes, rootLevel) {
 ...
 */
export function convertMdastToNoteHierarchy(mdast, text) {
    const seqCounter = { value: 0 };
    const allNotes = [];
    const rootChildrenBody = [];
    // normalise children from either MdastRoot (typed) or MdastNode (local)
    const mdast_children = (mdast.children || []);
    const rootEndOffset = mdast.position?.end?.offset ?? text.length;
    // Find all child notes from the MDAST root
    findChildNotes('root', mdast_children, text, seqCounter, rootEndOffset, allNotes, rootChildrenBody);
