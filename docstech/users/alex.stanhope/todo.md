...
+ what to publish
  + the VS Code web extension bundle: `client/extension/dist/extension.js`
  + the webview bundle: `client/webview/dist/index.js` + `client/webview/dist/public/`
  + the templates: `templates/getting-started.md`
  + the root `package.json` (this IS the VS Code extension manifest — has `browser`, `contributes`, `activationEvents`)
+ [X] add `files` array to root `package.json`
  + ```json
    "files": [
    ...
    ```
  + this tells npm which files to include in the tarball
  + root `package.json` is always included automatically (it's the extension manifest)
...
+ [X] add `prepublishOnly` script to root `package.json`
  + `"prepublishOnly": "pnpm run package"` (runs production webpack build before publish)
  + `package` script already exists: `webpack --mode production --devtool hidden-source-map`
+ [X] update `repository.url` if needed
  + currently `https://github.com/ZoomBuzz/NoteThink.git`
  + GitHub Packages requires this matches the repo — check casing (`zoombuzz` vs `ZoomBuzz`)
+ [X] remove dulcet-specific code from `package.json` contributes
  + `dulcet.saveToGitHub` command, `dulcet.openFromGitHub` command
  + `dulcet.openFile` configuration
  + `dulcet.active` keybinding conditions
  + these belong in the dulcet extension (notegit repo `extensions/dulcet/`), not in NoteThink
  + also remove from extension source (`dulcetSaveToGitHub.ts`, `dulcetOpenFromGitHub.ts`, `githubFileSystemProvider.ts`)
        ...
        steps:
          ...
          - name: Check if version is already published
            ...
            run: |
              ...
              if npm view @zoombuzz/notethink@$PACKAGE_VERSION version 2>/dev/null; then
          ...
          - name: Install dependencies
            ...
            run: |
              npm install
              cd client/extension && npm install && cd ../..
              cd client/webview && npm install && cd ../..
          - name: Publish
            if: steps.check.outputs.published == 'false'
            run: npm publish
            env:
              NODE_AUTH_TOKEN: ${{ secrets.GITHUB_TOKEN }}
    ```
  + identical pattern to `calfam-shared`, but install step also runs sub-package installs
+ [X] test locally
  + `npm pack` — verify tarball contains the right files
  + check tarball size is reasonable (extension.js + webview index.js + templates)
+ consumers
  + `nextjs/dulcet/package.json` adds `"@zoombuzz/notethink": "^0.1.25"` as dependency
  + dulcet's `scripts/copy-vscode-web.mjs` (or a new script) copies from `node_modules/@zoombuzz/notethink/` to `public/extensions/notethink/`
  + `.npmrc` in dulcet needs `@zoombuzz:registry=https://npm.pkg.github.com` (already needed for calfam-shared)
