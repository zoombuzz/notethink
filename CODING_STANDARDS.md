...
**Implementer rule:** transient code uses the implicit derived id and writes nothing to the file. Write an explicit `id=` linetag only when a durable artifact (a `[[…]]` cross-reference, a user-authored link) needs to survive across sessions or renames. NoteThink itself does not resolve a `[[…]]` reference: the parser is `mdast-util-from-markdown` with the GFM and frontmatter extensions and no wiki-link extension, so `[[x]]` renders as literal text and a reader follows it by searching the file for `?id=x` or the heading it derives from. See [AUTHORING_GUIDE.md - Stable ids: implicit vs explicit](./AUTHORING_GUIDE.md#stable-ids-implicit-vs-explicit) for the full decision rules. (Corrected 2026-09-11: this treated a `[[…]]` cross-reference as something NoteThink acts on.)
...
Why this matters: the view aggregates from N source files (folder mode); the editor only ever has one active doc. Driving view-interaction state through the editor selection alone silently breaks the moment the rendered tree contains anything outside that single doc - the round-trip never confirms and the view never updates. Pinning view focus on the most-recent view click instead silently breaks the moment the user starts editing in the editor - the visualisation goes stale. The fix is structural: write view state directly from the user's gesture for immediate feedback, then let the editor-derived match override as soon as it has an answer. See `client/webview/src/notethink-views/src/components/views/generic/useViewHandlers.ts` (the click dispatcher writes `view_focused_ids` / `view_selected_ids` directly, through `writeViewInteractionState` in `lib/viewstateops.ts`, which calls `setViewManagedState`) and `useViewContext.ts` (its call to `resolveFocusedNote`, from `lib/noteops.ts`, prefers editor-derived; view-driven fills in when the editor has no opinion) for the canonical pattern. (Corrected 2026-09-11: this named the fields `view_focused_seqs` / `view_selected_seqs`, since replaced by the `stable_id` fields, and had the dispatcher write them through `setViewManagedState` itself rather than through `writeViewInteractionState`.)
...
There is no `styles/` directory: styles are CSS modules beside the component they style, e.g. `components/Spinner.module.scss`. (Corrected 2026-09-11: the tree ended in a `styles/variables.scss` that exists nowhere in the repo.)
...
- **Rolling buffer.** The last `LOG_BUFFER_MAX` lines (2000 as at 2026-09-11), flushed `LOG_FLUSH_MS` (~1s) after a write, written wholesale (overwritten each flush, not appended); both constants are at the top of `client/extension/src/lib/errorops.ts`. (Corrected 2026-09-11: this said 500 lines.)
...
### No web dev server
notethink is a VS Code extension - there is no `pnpm run dev` and no HTTP server to start. The webview/extension bundles are produced by webpack (`pnpm run build` or `pnpm run watch`) and previewed in an ordinary VS Code window that loads the working copy through a per-machine `~/.vscode/extensions/notethink-dev` symlink, or manually through the F5 Extension Development Host; `AGENTS.md` > Dev Server has the detail. What the `/open-dev` skill's special case says is that notethink has no port to probe and no browser page to open. It is **not** exempt from the workspace dev-server start pattern: `/open-dev` launches `pnpm run watch` with the workspace launcher, `systemd-run --user --scope --slice=devservers.slice`, and workspace `AGENTS.md` > Dev servers names zahara as its only exception. (Corrected 2026-09-11: this said the skill exempted notethink from that pattern, and named the Extension Development Host as the preview.)
...
1. **Identify the live code path first** when the component has multiple branches. Example: the breadcrumb trail has two independent code paths - a single-file `splitPathSegments` branch (used when the toolbar shows "Current file") and a directory-aggregate `integration_path` branch - and they live in `breadcrumbSegmentsForView` in `notethink-views/src/lib/pathops.ts`, not in `BreadcrumbTrail.tsx`, which calls it for the segments. Determine which one is live from the current screenshot/state before editing, otherwise you'll edit the wrong branch and the visible behaviour will not change. (Corrected 2026-09-11: this placed both branches in `BreadcrumbTrail.tsx`.)
...
- match CI's pnpm major - both workflows run `pnpm/action-setup@v4` with no `version` input, so CI installs whatever pnpm the root `package.json` `packageManager` field pins. (Corrected 2026-09-11: this said the workflows pin `pnpm/action-setup` to v9.) regenerating with a newer local pnpm can add format-only fields (e.g. `libc:`) that the CI pnpm may reject; when only the specifier moved, a one-line hand-edit of the `specifier:` line is the minimal, version-safe fix.
