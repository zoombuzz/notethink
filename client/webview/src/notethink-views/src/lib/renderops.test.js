 ...
 */
function headingNote(children, overrides = {}) {
    return {
        seq: 1,
        level: 1,
        type: 'heading',
...
function mdastText(value, startOffset, endOffset) {
    return {
        type: 'text',
        position: {
...
function mdastStrong(children, startOffset, endOffset) {
    return {
        type: 'strong',
        position: {
            start: { offset: startOffset, line: 1 },
...
describe('renderMarkdownNoteHeadline', () => {
    describe('strip_linetags', () => {
        ...
        it('preserves inline formatting (bold) before the linetag', () => {
            ...
            const children = [
                mdastText('some ', 4, 9),
                mdastStrong([mdastText('bold', 11, 15)], 9, 17),
