 ...
 */
export function buildNoteStyles(note, extra_classes) {
    const styles = [view_specific_styles.note].concat(extra_classes || []);
    if (note.focused) {
        styles.push(view_specific_styles.focused);
    }
    if (note.selected) {
        styles.push(view_specific_styles.selected);
    }
    if (note.display_options?.settings?.show_line_numbers) {
        styles.push(view_specific_styles.addGutter);
    }
    return styles;
}
/**
 ...
 */
export function headlineClickPosition(note) {
 ...
 */
export function bodyClickPosition(note) {
    const selectable = note.display_options?.deepest?.selectable_note;
    return {
 ...
 */
export function createNoteClickHandler(note, position) {
    return (event) => {
 ...
 */
export function buildChildNoteDisplayOptions(view_display_options, note, view) {
    return {
